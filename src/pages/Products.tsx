...
export default function Products() {
  ...
  const products: Product[] = [...productsData.strategicProducts].sort((a, b) => a.order - b.order)
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* 핵심 전략 제품 */}
      <section className="py-20 bg-white" id="strategic">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="heading-en text-sm text-accent mb-4">STRATEGIC PRODUCTS</h2>
            <h3 className="text-3xl font-bold">{lang === 'ko' ? '핵심 전략 제품' : 'Strategic Products'}</h3>
            <p className="section-subtitle mt-4">
              {lang === 'ko'
                ? '모빌리티 · 통신 · 반도체 산업을 위한 정밀 냉간단조 부품'
                : 'Precision cold-forged parts for the mobility, telecom and semiconductor industries'}
            </p>
          </div>
      ...
      <section className="py-20 bg-gray-50" id="applications">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {applications.map((a) => (
              <div key={a.no} className="bg-white p-8 rounded-2xl shadow-sm hover-card">
                <span className="text-4xl font-bold text-primary/20">{a.no}</span>
                <h4 className="text-xl font-bold mt-2 mb-3">{a.title[lang]}</h4>
